...
export default function LeadFormModal({ open, onOpenChange, onDone, settings }) {
  ...
  const checkDup = async (phone) => {
    ...
    setDup(data.duplicate ? data.matches : null);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900" data-testid="lead-form-modal">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 py-2">
          ...
          <div>
            <Label>Source</Label>
            <Select value={form.source} onValueChange={(v) => set("source", v)}><SelectTrigger data-testid="lead-source-select" className="mt-1"><SelectValue /></SelectTrigger>
              <SelectContent>{(settings?.sources || []).map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select>
          </div>
          <div>
            <Label>Priority</Label>
            <Select value={form.priority} onValueChange={(v) => set("priority", v)}><SelectTrigger data-testid="lead-priority-select" className="mt-1"><SelectValue /></SelectTrigger>
        ...
        <DialogFooter>
          ...
          <Button data-testid="lead-form-submit" onClick={save} disabled={saving} className="bg-blue-600 hover:bg-blue-700">{saving ? "Saving…" : "Create lead"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
